...
// int f(vi a , int prev1 , int prev2 , int idx , vector<vector<vector<int>>>& dp){
//     if(idx == a.size()) return 0;
//     if(prev1 != -1 && prev2 != -1 && dp[idx][prev1][prev2] != -1) return dp[idx][prev1][prev2];
//     int val = a[idx];
//     int ans1 , ans2;
//     ans1 = f(a , val , prev2 , idx + 1 , dp);
//     ans2 = f(a , prev1 , val , idx + 1 , dp);
//     if(prev1 != -1 && a[idx] > prev1) ans1++;
//     if(prev2 != -1 && a[idx] > prev2) ans2++;
//     if(prev1 != -1 && prev2 != -1)
//     return dp[idx][prev1][prev2] = min(ans1, ans2);
...
void solve(){
   ...
   return;
    ...
    // vector<vector<vector<int>>> dp1(n+1 , vector<vector<int>>(n+1 , vector<int>(n+1 , 0)));
    // int ans = f(a , -1 , -1 , 0 ,dp1);
    // cout << ans << endl;
    // return;
}
