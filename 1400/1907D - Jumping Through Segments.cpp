...
bool f(vector<pii> a , int k){
    ...
    rep(i , 0 , n){
        ...
        curr.first -= k;
        curr.second += k;
        // if(left <= curr.second || right >= curr.first){
        //     curr.first = max(curr.first , left);
        //     curr.second = min(curr.second , right);
        // }
        // else return false;

        if(right < curr.first || left > curr.second){
            return false;
        }
        else{
            curr.first = max(curr.first , left);
