...
void dfs(int node, vector<vector<int>> &adj, vector<int> &vis, set<int>& n1 , set<int>& n2 , bool flag){
    if(flag){
        n1.insert(node);
    }
    else n2.insert(node);
    ...
    for(auto nxt: adj[node]){
        if(!vis[nxt]) dfs(nxt, adj, vis , n1 , n2 , !flag);
    }
}
