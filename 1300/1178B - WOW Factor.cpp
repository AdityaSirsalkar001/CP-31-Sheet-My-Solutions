...
int f(string& s , int idx , int wow , vector<vector<int>> &dp){
    int n = s.size();
    if(idx >= n) return 0;
    if(dp[idx][wow] != -1) return dp[idx][wow];
    char c = s[idx];
    
    int ans = 0;
    ans += f(s , idx + 1 , wow , dp);
    if(c == 'o'){
        if(wow == 1) ans += f(s , idx + 1 , wow + 1 , dp);
    ...
    if(c == 'v'){
        if(idx < n - 1){
            ...
            if(d == 'v'){
                if(wow == 0) ans += f(s , idx + 2 , wow + 1 , dp);
                if(wow == 2) ans += 1; 
